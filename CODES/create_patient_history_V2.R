...
#................................
train_patient_IDs <- unique(train$Patient_ID)
train[,tag:="train"]
test[,tag:="test"]

test_new <- test[!(Patient_ID %in% train_patient_IDs),]
test_common <- test[(Patient_ID %in% train_patient_IDs),]

dat_outcome <- train[,c("Patient_ID","Health_Camp_ID","Outcome"),with=F]
test_common[,Outcome:=0]
test_common = subset(test_common, select = colnames(train))
dat_common <- rbind(train,test_common)

#................................
dat_common <- registration.around.campStart(df=dat_common)
dat_common <- num.repeated.registration(df = dat_common)
dat_common <- time.of.association(df=dat_common)
dat_common <- total.registrations.camp(df=dat_common)
dat_common <- patient.outcome.sum(df=dat_common)

dat_common$con_rate <- dat_common$patient_outcome_sum/ dat_common$registration_num_overall 
dat_common$con_rate[is.infinite(dat_common$con_rate)] <- NA
dat_common[,patient_outcome_sum:=NULL]

#................................
test_new <- registration.around.campStart(df=test_new)
test_new <- num.repeated.registration(df = test_new)
test_new <- time.of.association(df=test_new)
test_new <- total.registrations.camp(df=test_new)
test_new$con_rate <- NA

#................................
train = dat_common[tag=="train",]
test <- rbind(test_new, subset(dat_common,subset=tag=="test", select = c(-Outcome)))

#train <- merge(train, dat_outcome,by = c("Patient_ID","Health_Camp_ID"))
setorder(test,"Patient_ID","Registration_Date")
test[,tmp_ind:=1:.N,by="Patient_ID"]
s = test[tmp_ind==1 & !(is.na(con_rate)),c("Patient_ID","con_rate"),with=F]
setnames(s,"con_rate","con_rate2")
test = merge(test,s,by = c("Patient_ID"), all.x=T)
test[,con_rate:=NULL]
setnames(test,"con_rate2","con_rate")
test[,tmp_ind:=NULL]
